...
def swap_spe_features(type_list, value_list):
    ...
    for keys in spe_dict.keys():
        random.shuffle(spe_dict[keys])#将同一伪造方法的图打乱顺序
...
class ContrastiveLoss(AbstractLossClass):
    ...
    def forward(self, common, specific, spe_label):
        ...
        # 16 common.shape[0]
        real_common, fake_common = common.chunk(2)#没打乱
        # common real
        idx_list = list(range(0, bs//2))
        random.shuffle(idx_list)
        # print(idx_list, 'idx_list')
        # print(common, 'common')
        real_common_anchor = common[idx_list]#打乱顺序
        # common fake
        idx_list = list(range(bs//2, bs))
        random.shuffle(idx_list)
        fake_common_anchor = common[idx_list]#打乱顺序
        # specific
        ...
        loss_realcommon = self.contrastive_loss(
            real_common, real_common_anchor, fake_common_anchor)#real_common没打乱 和同种共同之间差距越小越好 和不同（假）之间差距越大越好
        loss_fakecommon = self.contrastive_loss(
            fake_common, fake_common_anchor, real_common_anchor)#fake_common没打乱 和同种伪造方法之间差距越小越好 和不同（真）之间差距越大越好

        # Comupte the constrastive loss of specific between real and fake
        loss_realspecific = self.contrastive_loss(
            real_specific, real_specific_anchor, fake_specific_anchor)#real_specific没打乱 和同种伪造方法之间差距越小越好 和不同（假）之间差距越大越好
        loss_fakespecific = self.contrastive_loss(
            fake_specific, fake_specific_anchor, real_specific_anchor)#fake_specific没打乱 和同种伪造方法之间差距越小越好 和不同（真）之间差距越大越好
