...
def Stage1(dim, index, layers, mlp_ratio=4.,
          act_layer=nn.GELU,
          drop_rate=.0, drop_path_rate=0.,
          use_layer_scale=True, layer_scale_init_value=1e-5, vit_num=1):
    """
...
class SwiftFormer(nn.Module):
    ...
    def __init__(self, layers, embed_dims=None,
        ...
        self.patch_embed = stem(384, embed_dims[0])
        self.patch_embed1 = stem(2048, embed_dims[0])
        ...
        network1 = []
        for i in range(len(layers)):
            stage = Stage1(embed_dims[i], i, layers, mlp_ratio=mlp_ratios,
                          act_layer=act_layer,
                          drop_rate=drop_rate,
                          drop_path_rate=drop_path_rate,
                          use_layer_scale=use_layer_scale,
    ...
    #     self.pag1 = PagFM(512, 256)
    #
    #     self.pag2 = PagFM(256, 128)
    #
    #     self.pag3 = PagFM(128, 64)
    #
    #     self.pag4 = PagFM(64, 32)
    ...
    def forward_tokens(self, x,y):
        outs = []
        # for idx, block in enumerate(self.network):
        #     x = block(x)
        # c=0
        for idx, (block, block1) in enumerate(zip(self.network, self.network1)):
            x = block(x)
            y = block1(y)
            # print(idx,x.shape)
            # print(idx, y.shape)
...
# @BACKBONE.register_module(module_name="SwiftFormer_S")
# def SwiftFormer_S(edge_config=None,pretrained=False, **kwargs):
#     model = SwiftFormer(
...
@BACKBONE.register_module(module_name="SwiftFormer_fusion")
def SwiftFormer_fusion(edge_config=None,pretrained=False, **kwargs):
    model = SwiftFormer(
